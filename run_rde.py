...
if __name__ == "__main__":
    ...
    parser.add_argument('-bs', '--batchsize', type=int, default=50, help='Batch size for GS and computing features')
    ...
    for dataset in args.dataset:
        ...
        filename = os.path.join(dir_name, f"{dataset}_rde_")

        if not args.no_logger:
            logger = Logger(os.path.join(dir_name, f"{dataset}_rde_log.txt"))
        ...
        if args.linsvc:
            param_grid = {**CONFIG_RDE_GS, **CONFIG_PRE_GS, **CONFIG_LIN_SVC_GS}
        else:
            param_grid = {**CONFIG_RDE_GS, **CONFIG_PRE_GS, **CONFIG_SVC_GS}
