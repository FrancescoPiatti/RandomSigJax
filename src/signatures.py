...
@partial(jax.jit, static_argnames=('level', 'chunk_size'))
def _sig_chunks_fixed(X: jnp.ndarray,
                         starts: jnp.ndarray,   # (n_chunks,) int32
                         level: int,
                         chunk_size: int) -> jnp.ndarray:
    """
    ...
    def one_chunk(start):
        ...
        return signature(x_chunk, level)            # (B, D)
...
def get_signatures(
    ...
    """
    Compute per-chunk signatures along time.
    All chunks have length==step, except possibly the last one.
    Returns: (B, n_chunks, D)
    ...
    def run_one_batch(Xb: jnp.ndarray) -> jnp.ndarray:
        outs = []
        if n_full > 0:
            outs.append(_sig_chunks_fixed(Xb, starts_full, order, step))      # (b, n_full, D)
        if last_len is not None and last_len > 0:
            outs.append(_sig_chunks_fixed(Xb, starts_last, order, last_len))  # (b, 1, D)
        return outs[0] if len(outs) == 1 else jnp.concatenate(outs, axis=1)
